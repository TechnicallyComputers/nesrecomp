...
#define AUDIO_MAX_SAMPLES_PER_FRAME 900
#define AUDIO_SOURCE_RATE        44100.0
static int16_t            s_audio_frame[AUDIO_MAX_SAMPLES_PER_FRAME];
...
smoke_skip_input:
    ...
    if ((g_ppuctrl & 0x80) && runtime_get_vblank_depth() > 1) {
        if (g_nested_nmi_policy == NESTED_NMI_RUN_HANDLER) {
             ...
             * A game may explicitly opt in to an additional flag address;
             * Tetris uses $33, which is unrelated state in other titles. */
    ...
    if (s_audio_dev && !turbo_active() && !headless_run_active() && !s_rb_tick_replay) {
        ...
        if (s_have_boundary && g_frame_boundary_cyc > s_last_boundary_cyc) {
            ...
            if (delta >= 20000 && delta <= 45000) {
                ...
                if (push_n > AUDIO_MAX_SAMPLES_PER_FRAME)
                    push_n = AUDIO_MAX_SAMPLES_PER_FRAME;
                /* Clamping an outlier must not create an unbounded sample
                 * debt. Carry only the fractional part into the next frame. */
                s_audio_sample_carry = want - (double)(int)want;
            }
        }
