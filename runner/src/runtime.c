...
/* Retained for projects adopting PR 21. The renderer consumes native NES
 * PPUMASK bits, so enabling this compatibility flag must not remap them. */
...
static inline uint8_t nes_ppumask_to_runner(uint8_t val) {
    /* Both layouts are native: BG=0x08, sprites=0x10, left enables=0x02/0x04.
     * Preserve greyscale and emphasis bits as well. */
    return val;
}
...
static uint16_t s_miss_last_cpu_addr = 0;
...
static int      s_dispatch_miss_dumps = 0;
...
int g_nested_nmi_extra_flag_addr = -1;

void nes_resolve_nested_nmi_spin_flags(void) {
    g_ram[0x1A] = 1;
    g_ram[0x20] = 1;
    if (g_nested_nmi_extra_flag_addr >= 0 &&
        g_nested_nmi_extra_flag_addr < 0x800)
        g_ram[g_nested_nmi_extra_flag_addr] = 1;
}
...
void nes_write_runtime_fault(const char *reason) {
    char path[300];
    snprintf(path, sizeof(path), "%sruntime_faults.log", g_exe_dir);
    ...
    fprintf(out,
            ...
            "last_miss gen=$%04X cpu=$%04X bank=%d class=%s call_site=$%04X\n",
            ...
            g_miss_last_addr, s_miss_last_cpu_addr, g_miss_last_bank,
            s_last_miss_class, s_last_miss_ctx);
#ifdef RECOMP_STACK_TRACKING
    fprintf(out, "recomp_stack top=%d last=%s\n", g_recomp_stack_top,
            g_last_recomp_func ? g_last_recomp_func : "(none)");
    int first = g_recomp_stack_top > 32 ? g_recomp_stack_top - 32 : 0;
    for (int i = g_recomp_stack_top - 1; i >= first; i--)
        fprintf(out, "  [%d] %s\n", i,
                g_recomp_stack[i] ? g_recomp_stack[i] : "?");
#endif
    fprintf(out, "last_dispatches\n");
    write_dispatch_ring(out, 128);
    fprintf(out, "=== end runtime fault ===\n\n");
    fclose(out);
}
...
#ifdef _MSC_VER
    if (addr == 0 && getenv("NESRECOMP_BRK_CTX")) {
...
void nes_record_dispatch_miss_bank(uint16_t addr, uint16_t cpu_addr, int bank) {
    ...
    /* Persist bounded context for both legitimate discovery misses and invalid
